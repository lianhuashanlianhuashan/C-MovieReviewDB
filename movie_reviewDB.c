 ...
 * 1011119576
 * Student Name (Last name, First name):Gao Yuwei
 * Student Number:1011119576
 * UTORid:gaoyuwe1
 ...
 * 
 * Sign here with your name:Yuwei Gao
 *
 *  
 */
...
ReviewNode *newMovieReviewNode()
{
    ...
    if (new_node==NULL){
        printf("No memory available for newMovieReviewNode().\n");
        return NULL;
    }
...
void updateMovieReview(char title[MAX_STR_LEN], char studio[MAX_STR_LEN], int year, float BO_total, int score, ReviewNode *head)
{
	...
	if (findMovieReview(title, studio, year, head)==NULL){
		...
		ReviewNode *update=findMovieReview(title, studio, year, head);
...
ReviewNode *deleteMovieReview(char title[MAX_STR_LEN], char studio[MAX_STR_LEN],int year, ReviewNode *head)
{
	...
	if (findMovieReview(title, studio, year, head)==NULL){
		return head;
	}else if (findMovieReview(title, studio, year, head)==head){
		ReviewNode *new=head->next;
		free(head);
		return new;
	}else{
			int found=0;
...
void insertCastMember(char title[MAX_STR_LEN], char studio[MAX_STR_LEN], int year, ReviewNode *head, char name[MAX_STR_LEN], float salary)
{
   ...
   */   
	...
	ReviewNode *rev=findMovieReview(title, studio, year, head);           //rev points to the target reviewnode
	if (rev!=NULL){
		CastList *p=rev->review.cast;                                                    //p points to the target cast head
		CastList *new=(CastList *)calloc(1, sizeof(CastList));
		if (new==NULL){return;}
		strcpy(new->name,name);
		new->salary=salary;
		new->next=NULL;
		if (p==NULL){rev->review.cast=new;
		}else{
			while(p->next!=NULL){
				p=p->next;
			}
...
float salary(ReviewNode *head, char name[MAX_STR_LEN]){               //return the average salary of a cast member
	int movie=0;
	float money=0.0f;
	while (head!=NULL){
		CastList *lst=head->review.cast;
		while(lst!=NULL){
...
int actorIn(ActorNode *actor, char name[MAX_STR_LEN]){
	int in=0;
	while (actor!=NULL){
...
ActorNode *addActor(ActorNode *actor, char name[MAX_STR_LEN]){
	ActorNode *new=(ActorNode *)calloc(1, sizeof(ActorNode));
	if(new!=NULL){
		strcpy(new->name, name);
		new->next=NULL;
	}else{
		printf("no space for addActor()");
		return NULL;
	}
	new->next=actor;
	return new;
...
void whosTheStar(ReviewNode *head)
{
   ...
   */
	...
	char name[MAX_STR_LEN];
	float average=0.0f;
	ActorNode *actor=NULL;                                       //head of the list of checked actors
	CastList *cast=NULL;
	while (head!=NULL){
		cast=head->review.cast;
		while(cast!=NULL){
			if (actorIn(actor, cast->name)==0){
				if(salary(head, cast->name)>average){
					strcpy(name, cast->name);
					average=salary(head, cast->name);
					actor=addActor(actor, cast->name);
				}
			}
			cast=cast->next;
		}
